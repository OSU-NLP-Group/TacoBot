...
class ColBERTInfiller(Annotator):
    name='colbertinfiller'
    ...
    def execute(self, input_data: Optional[Dict] = None):
        """
        Uses pretrained ColBERT to infill prompt templates
        ...
        logger.primary_info(f'Calling ColBERT contextual infilling module with data="{json.dumps(input_data)}"')

        output = super().remote_call(input_data)
        logger.primary_info(f"ColBERT contextual infilling module returned output: {output}")
...
if __name__ == "__main__":
    ...
    class TestModule:
        ...
        def execute(self, data):
            ...
            return response
    module = TestModule("http://ec2-3-214-217-42.compute-1.amazonaws.com") # TODO (eric): REPLACE THIS WITH SOMETHING MEANINGFUL
    output = module.execute({"prompts": ["Yeah, I love [person] too!"], "contexts": ["i love taco"]}).json()
    print(output)