'''
Out-of-Context QA to replace EVI
'''
import logging
...
class OOC_response(Annotator):
    ...
    def execute(self, input_data=None):
        ...
        logger.debug(f'Calling OOC_response Remote module with utterances="{user_utterances}"')

        OOC_rp_output = self.remote_call(input_data)
        if OOC_rp_output is None:
