...
from taco.response_generators.opinion2.state_actions import Action, State, AdditionalFeatures
from taco.response_generators.opinion2.abstract_policy import Policy

class DisagreeAgreeSwitchAgreePolicy(Policy):

    def __repr__(self) -> str:
        return "DisagreeAgreeSwitchAgreePolicy"

    def disagree_solicit_reason(self, state : State, action_space : List[Action]):
    ...
    def agree_solicit_agree(self, state : State, action_space : List[Action]):
    ...
    def agree_suggest_alternative(self, state : State, action_space : List[Action]):
        action = Action(sentiment=4 - state.cur_sentiment, suggest_alternative=True)
        if action not in action_space:
