...
import taco.core.blacklists.blacklists as blacklists
...
class Phrase:
    ...
    def from_row(row : List[str]):
        text, category, wiki_entity_name, wiki_category, good_for_wiki = row
        return Phrase(text, category, wiki_entity_name, wiki_category, good_for_wiki, False)
...
def get_opinionable_phrases() -> List[Phrase]:
    # entries = fetch_sql("select phrase, category, wiki_entity_name, wiki_entity_name, good_for_wiki, generic from labeled_phrases;")
    entries = fetch_sql("select phrase, category, wiki_entity_name, wiki_entity_name, good_for_wiki from labeled_phrases;")
    # cur.execute(b'insert into labeled_phrases (phrase, category, wiki_entity_name, good_for_wiki, creation_date_time) values
...
def get_opinions(phrase : str) -> Set[Opinion]:
    ...
    :rtype: Set[Opinion]
    """
    query = "select distinct entity, reason, attitude, sentiment from labeled_opinions where entity = %s and reason_appropriateness = 4"
