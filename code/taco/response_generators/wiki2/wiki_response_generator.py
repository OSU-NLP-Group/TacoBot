...
from taco.core.util import run_module, killable
...
from taco.response_generators.wiki2.wiki_utils import token_overlap
import taco.response_generators.wiki2.wiki_utils as wiki_utils
from taco.response_generators.wiki2.blacklists import CATEGORY_BLACK_LIST, ENTITY_BLACK_LIST
...
from taco.core.response_generator_datatypes import *
from taco.response_generators.wiki2.treelets import *
from taco.response_generators.wiki2.wiki_helpers import *
from taco.response_generators.wiki2.wiki_infiller import *
# from taco.response_generators.wiki2.wiki_utils import *
from taco.response_generators.wiki2.pronouns import get_pronoun
from taco.core.smooth_handoffs import SmoothHandoff
from taco.response_generators.wiki2.response_templates.response_components import *
from taco.annotators.corenlp import Sentiment
from taco.response_generators.wiki2.state import State,ConditionalState, NO_UPDATE
...
try:
    ...
    logger.warning('ResponseRanker module not found, defaulting to original DialoGPT and GPT2 Rankers')
    from taco.annotators.dialogptranker import DialoGPTRanker
    from taco.annotators.gpt2ranker import GPT2Ranker
...
class WikiResponseGenerator(ResponseGenerator):
    ...
    def handle_current_entity(self, priority=ResponsePriority.CAN_START):
        ...
        # if answer_type == AnswerType.QUESTION_CT_INGRED:
        #     self.utterance
        #     user_initiated_enitiy, user_has_posnav = get_user_initiated_entitiy(self.utterance, self.state_manager.current_state)
        #     response = None
    ...
    def handle_smooth_handoff(self) -> Optional[PromptResult]:
        ...
        logger.primary_info(f'cur_entity = {cur_entity}')
    ...
    def get_untalked_entity(self):
        ...
        ingre_untalked_entities = self.get_entities()['ingredient_untalked']
        for entity in reversed(ingre_untalked_entities):
        ...
        pad_untalked_entities = self.get_entities()['pad_untalked']
        for entity in reversed(pad_untalked_entities):
            if (not entity.is_category) and (not (entity in ENTITY_BLACK_LIST)) and \
                    (not (entity.name in state.entity_state or state.entity_state[entity.name].finished_talking)):
                logger.primary_info(f"Found {entity} in entity_tracker.user_mentioned_untalked, the latest entity"
                                    f"for Wiki to not have finished talking")
    ...
    def get_intro_prompt(self) -> Optional[PromptResult]:
        ...
        #     if not (turns_since_last_active['WIKI'] >= 4 and turns_since_last_active['TRANSITION'] >= 8):
        ...
        if not (turns_since_last_active['WIKI'] >= 5):
            return self.emptyPrompt()
