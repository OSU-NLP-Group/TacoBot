...
from taco.annotators.responseranker import ResponseRanker
from taco.core.response_generator.treelet import Treelet as BaseTreelet
from taco.core.response_generator_datatypes import ResponsePriority, emptyResult_with_conditional_state
from taco.core.response_generator_datatypes import ResponseGeneratorResult, PromptResult, AnswerType
from taco.response_generators.neural_chat.state import State, ConditionalState, UserLabel, BotLabel
from taco.response_generators.neural_fallback.neural_helpers import neural_response_filtering
from taco.response_generators.neural_chat.util import MAX_NUM_NEURAL_TURNS, NEURAL_DECODE_CONFIG, is_two_part, question_part, NUM_CONVO_OPENING_TURNS, BLACKLIST, is_short, is_short_set
...
from taco.core.util import remove_punc, get_ngrams
from taco.core.regex.util import OPTIONAL_TEXT_PRE, OPTIONAL_TEXT_POST, OPTIONAL_TEXT_MID, oneof
from taco.core.entity_linker.entity_groups import EntityGroup
...
class Treelet(BaseTreelet):
    ...
    def optionally_quit_conv(self, state: State) -> Optional[ResponseGeneratorResult]:
        ...
        if nav_intent.pos_intent and not nav_intent.pos_topic_is_current_topic:
            logger.taco_merge(f'User has PosNav intent for something other than the current topic, so Neural Chat {self.name} is stopping talking')
        ...
        if nav_intent.neg_intent or (self.use_neg_answer_as_negnav and self.state_manager.current_state.dialogact['is_no_answer']):
            logger.taco_merge(f'User has NegNav intent (or possibly neg answer), so giving handover response with needs_prompt=True.')
    ...
    def choose_best_neural_response(self, responses: List[str], history: List[str], state: State, scores: Optional[List[float]] = None):
        ...
        #logger.taco_merge(f'Of the {len(responses)} neural responses, {num_questions} are questions, so enough_questions={enough_questions}')
        ...
        logger.taco_merge("Got the following responses: {}".format("\n".join([str(x) for x in zip(responses, scores)])))
        ...
        if len(responses) == 0:
            ...
            logger.taco_merge('Remaining neural responses with scores after editing, sorted by preference:\n' +
        ...
        if best_response is None: # If the model returned zero responses, hand over and report a technical error.
            ...
            if self.is_topic_shifting_question(best_response):
                logger.taco_merge("Best response is a topic-shifting question; incrementing by 1.")
    ...
    def cutoff_questions(self, best_response: str, state: State, remove_topic_shifts : bool = True) -> str:
        ...
        #    logger.taco_merge(f'Trimming question out of final neural chat turn, yielding {best_response}')
            ...
            if self.is_question(sentence):
                ...
                if limit == 0 or (self.is_topic_shifting_question(sentence) and remove_topic_shifts):
                    if (not is_short_set(kept_sentences)): # try not to produce an empty sentence ever if possible
                        logger.taco_merge(f"Not adding on sentence: {sentence} ({self.is_topic_shifting_question(sentence)})")
                        break
    ...
    def rerank_responses(self, response_score_pairs: List[Tuple[str, float]], responses: List[str], history: List[str], state: State) -> List[Tuple[str, float]]:
        ...
        logger.taco_merge(f'{len(responses)} suitable responses; retrieving ranking scores using policy "{policy}"')
    ...
    def should_stop(self, bot_response: str, state: State) -> bool:
        ...
        #    logger.taco_merge(f'The neural response does not contain a question, so ending {self.name} conversation.')
        #    return True
        if self.is_topic_shifting_question(bot_response) and state.num_topic_shifts >= 0:
            logger.taco_merge(f"Already encountered {state.num_topic_shifts} topic shifts, so stopping now.")
            return True
        if self.too_many_turns(state):
            logger.taco_merge(f'We have exceeded the max num neural turns {MAX_NUM_NEURAL_TURNS}, so leaving {self.name} conversation.')
            return True
        return False
    ...
    def has_recommended_wiki_entity(self, state: State):
        ...
        if recommended_entity:
            if recommended_entity.is_category:
                ...
                logger.taco_merge(f"Neural chat handoff: observing recommended entity {recommended_entity}: {recommended_entity.name}. Handing off to WIKI.")
    ...
    def get_response(self, state: State, force: bool = False) -> ResponseGeneratorResult:
        ...
        #         logger.taco_merge(f"{self.name} detected user asking return question, but we've already given the return answer, so letting neural module respond alone")
        #     else:
        #         logger.taco_merge(f'{self.name} detected user asking return question, so prepending return question answer to neural module response')
        ...
        logger.taco_merge(f'get_response bot_labels = {bot_labels}')
        ...
        if BotLabel.HANDOVER in bot_labels:
            smooth_handoff = None
            logger.taco_merge(f'Bot response has HANDOVER label, so appending fallback response as a wrapup phrase and exiting {self.name} conversation.')
            fallback_response = self.get_fallback_response(state, history)
            if bot_response:
                # not using wiki entity
