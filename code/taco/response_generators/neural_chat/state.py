...
class State(object):
    should_go_back_instruction = False
    response_types = None
    most_recent_treelet=""
    ...
    def treelet_has_been_used(self, treelet_name: str) -> bool:
        ...
        if treelet_name == 'GO_BACK_Treelet':
            # exclude go back treelet
            return True
        elif treelet_name not in self.conv_histories:
            return False
        else:
    ...
    def update_if_chosen(self, conditional_state: ConditionalState):
        ...
        if conditional_state.next_treelet != NO_UPDATE:
            # next_treelet would be assigned when initialing conditional_state
            self.next_treelet = conditional_state.next_treelet
        # Update the ConvHistory for most_recent_treelet
        # 'TOPIC_SHIFT', 'HANDOVER'
        # if handover go back to main task

        # print('conditional_state = ', conditional_state)
        # logger.taco_merge(f'conditional_state.bot_labels = {conditional_state.bot_labels}')
        self.num_time_conversation += 1

        if 'HANDOVER' in conditional_state.bot_labels or self.num_time_conversation > 5:
            
            logger.taco_merge(f'update_if_chosen conditional_state.bot_labels = {conditional_state.bot_labels}')

            self.should_go_back_instruction = True
            self.num_time_conversation = 0
            
        self.most_recent_treelet = conditional_state.most_recent_treelet
    ...
    def update_if_not_chosen(self, conditional_state: ConditionalState):
        ...
        if self.num_time_conversation > 5:
            self.should_go_back_instruction = True
            self.num_time_conversation = 0
