...
class IngredientTreelet(Treelet):
    ...
    def get_response(self, state_manager, priority=ResponsePriority.WEAK_CONTINUE, **kwargs) -> ResponseGeneratorResult:
        ...
        speak_output = ''
        if not is_wikihow:
            # logger.taco_merge(f"recipe_query_result[documents].keys() = {recipe_query_result.keys()}")
            # logger.taco_merge(f'list_item_selected = {list_item_selected}')
            ...
            matched_ingredients, _ = TacoIntentByRule.match_ingredients(user_utterance, ingredients, is_all=False)
            # logger.taco_merge(f'[ingredients list] matched: {matched_ingredients}')
