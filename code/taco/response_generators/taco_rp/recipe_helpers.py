...
def _get_ingredients(raw_ingredients):
    ...
    for ingredient in raw_ingredients:
        ...
        if (
            ('unit' in ingredient and ingredient["unit"] not in unit_table)
            or ('ingredient' in ingredient and not ingredient["ingredient"])
            or ('quantity' in ingredient and not ingredient["quantity"])
        ):
            helpers.logic_exception("Ingredient quantity missing from unit table: " + ingredient["displayText"])
            ingredients.append(ingredient["displayText"])
            continue

        quantity = ingredient["quantity"] if "quantity" in ingredient else 0
        unit = unit_table[ingredient["unit"]] if "unit" in ingredient else None
        name = ingredient["ingredient"] if "ingredient" in ingredient else None
        ...
        if quantity_char is None:
            helpers.logic_exception("Ingredient missing from unicode table:%s"%(ingredient))
            ingredients.append(ingredient["displayText"])
            continue
