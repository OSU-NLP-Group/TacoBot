...
# treelets
from taco.response_generators.taco_rp.list.treelets.taco_list_treelet import Taco_list_Treelet

from taco.response_generators.taco_rp.list.state import State, ConditionalState

NAME2TREELET = {treelet.__name__: treelet for treelet in [Taco_list_Treelet]}

print('NAME2TREELET = ', NAME2TREELET)

# response type
ADDITIONAL_RESPONSE_TYPES = ['NEEDED_list']
ResponseType = add_response_types(ResponseType, ADDITIONAL_RESPONSE_TYPES)
        
logger = logging.getLogger('tacologger')


class list_Generator(ResponseGenerator):
    name='list_INTERNT'

    def __init__(self, state_manager) -> None:
        self.list_treelet = Taco_list_Treelet(self)
        
        treelets = {
            treelet.name: treelet for treelet in [self.list_treelet]
        }
    ...
    def handle_default_post_checks(self) -> Optional[ResponseGeneratorResult]:
        ...
        treelet = name2initializedtreelet['Taco_list_Treelet']
        response = treelet.get_response(self.state_manager, force=True)
        logger.primary_info(f"list_INTERNT Response is: {response}")
        return response
