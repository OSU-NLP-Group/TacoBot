...
def taco_recipe_choice(current_state, last_state, user_attributes, toolkit_service_client):
    ...
    if (recipe_query_result is not None and 
        ...
        # logger.taco_merge(f'in recipes =  {repr(recipes)[:200]}')
    else:
...
def select_recipe_template(query, recipes, num_results, choice_start_idx, current_state, user_attributes):
    ...
    speak_output = (
        select_segment1(query, first_visit, is_rec, current_state, current_state.last_status) + 
        (
            select_segment2_w_constraints(choice_start_idx, query, num_results, recipes, satisfied_constraints, current_state)
            if current_state.last_status and 'TaskClarification' in current_state.last_status
...
def select_segment1(query, first_visit, is_rec, current_state, last_status):
    ...
    if first_visit:
        ...
        elif last_status and 'TaskClarification' not in last_status:
...
def select_segment2(choice_start_idx, is_rec, num_results, recipes):
    ...
    if choice_start_idx == 0:
        if is_rec:
            ...
            speak_output = "I found only one recipe for you. "
        elif num_results == 2:
            speak_output = 'I only found two recipes for you. '
            if recipes[0].name == recipes[1].name:
                speak_output += f'They are both called {recipes[0].name}. '
        else:
            speak_output = 'I found several recipes for you. '
...
def select_segment2_w_constraints(choice_start_idx, query, num_results, recipes, satisfied_constraints, current_state):
    ...
    if satisfied_constraints is None or not clarify:
        if intent == 'ClarifyIntent':
            speak_output = f"You said {text}, but I can't understand that as a nutrition constraint. "
        ...
        if choice_start_idx == 0:
            if num_results == 1:
                speak_output += f"Here is only one {query} recipe for you. "
            elif num_results == 2:
                speak_output += f'Here are the only two {query} recipes for you. '
                if recipes[0].name == recipes[1].name:
                    speak_output += f'They are both called {recipes[0].name}. '
            else:
                speak_output += f'Here are the top {query} recipes for you. '
                if recipes[0].name == recipes[1].name and recipes[0].name == recipes[2].name:
                    speak_output += f'They are all called {recipes[0].name}. '
    else:
        attr_str = get_and_list_prompt(satisfied_constraints)
        ...
        if choice_start_idx == 0:
            if num_results == 1:
                speak_output = f"I found only one {query} recipe that is {attr_str} for you. "
            elif num_results == 2:
                speak_output = f'I only found two {query} recipes that are {attr_str} for you. '
                if recipes[0].name == recipes[1].name:
                    speak_output += f'They are both called {recipes[0].name}. '
            else:
                speak_output = f'I found several {query} recipes that are {attr_str} for you. '
                if recipes[0].name == recipes[1].name and recipes[0].name == recipes[2].name:
                    speak_output += f'They are all called {recipes[0].name}. '
