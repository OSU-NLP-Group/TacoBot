...
def get_constraints(current_state):
    ...
    if recipesearch and 'satisfied_constraints' in recipesearch:
        tmp_satisfied_constraints = recipesearch['satisfied_constraints']
        if tmp_satisfied_constraints:
            ...
            if 'level' in tmp_satisfied_constraints:
                satisfied_constraints.append(tmp_satisfied_constraints['level'][0])
            if 'nutrition' in tmp_satisfied_constraints:
                satisfied_constraints.append(tmp_satisfied_constraints['nutrition'][0])

            print('[recipe choice] satisfied constraints: ', satisfied_constraints)
    return [' '.join(satisfied_constraints)]