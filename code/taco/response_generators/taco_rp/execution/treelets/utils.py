...
def count_step_and_check_exceptions(text, intent, current_state, user_attributes):
    ...
    method_idx, num_methods, total_steps, current_step = TacoIntentByRule.parse_step_nums(current_state.status)
    prev_method_idx, prev_num_methods, prev_total_steps, prev_step = TacoIntentByRule.parse_step_nums(current_state.last_status)
    ...
    if 'TaskPreparation' in current_state.last_status:
    ...
    if error_step_num is None and error_method_num is None and not getattr(current_state, 'resume_task', False):
        if (
            ...
            'Detail' not in current_state.last_status
        ):
...
def add_step_markers(current_step, total_steps, method_idx, num_methods, speak_output, current_state, user_attributes):
    has_parts = getattr(user_attributes, 'has_parts', False)
    
    if current_step == 0 and 'TaskPreparation' not in current_state.last_status:
        if has_parts:
...
def decorate_speak_output(current_state, user_attributes, speak_output, has_parts = False):
    method_idx, num_methods, total_steps, current_step = TacoIntentByRule.parse_step_nums(current_state.status)
    ...
    speak_output = add_step_markers(current_step, total_steps, method_idx, num_methods, speak_output, current_state, user_attributes)
