...
def taco_wikihow_show_steps(current_state, last_state, user_attributes):
    ...
    speak_output, document = get_wikihow_steps(current_task_docparse, current_state, user_attributes)
...
def get_wikihow_steps(current_task_docparse, current_state, user_attributes):
    """
    ...
    """
    method_number, _, total_steps, current_step = TacoIntentByRule.parse_step_nums(current_state.status)

    step_text = current_task_docparse[method_number][current_step]['qa_context']
    update_wikihow_user_attrs(current_state, user_attributes, current_task_docparse, step_text)
    ...
    speak_output = decorate_speak_output(current_state, user_attributes, speak_output, has_parts=user_attributes.has_parts)
...
def update_wikihow_user_attrs(current_state, user_attributes, current_task_docparse, step_text):
