...
def taco_wikihow_details(current_state, last_state, user_attributes):
    ...
    method_idx, N_methods, total_steps, current_step = TacoIntentByRule.parse_step_nums(current_state.status)
    ...
    if not is_apl_supported:
        ...
        document = get_details_apl_doc(current_state, user_attributes)
...
def taco_wikihow_tips(current_state, last_state, user_attributes):
    ...
    method_idx, N_methods, total_steps, current_step = TacoIntentByRule.parse_step_nums(current_state.status)
    tip_no = int(re.findall(r'tipNo(\d+)', current_state.status)[0])
    ...
    if not is_apl_supported:
        ...
        document = get_tips_apl_doc(current_state, user_attributes)
        return {'response': speaker_output, 
            'directives': [document], 
            'shouldEndSession': False}
...
def get_details_apl_doc(current_state, user_attributes):
    method_number, _, total_steps, step_number = TacoIntentByRule.parse_step_nums(current_state.status)
...
def get_tips_apl_doc(current_state, user_attributes):
    method_number, _, total_steps, step_number = TacoIntentByRule.parse_step_nums(current_state.status)
    tip_number = int(re.findall(r'tipNo(\d+)', current_state.status)[0])
    step_dict = user_attributes.current_task_docparse[method_number][step_number]
