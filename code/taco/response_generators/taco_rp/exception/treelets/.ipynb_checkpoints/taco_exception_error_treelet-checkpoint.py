...
#         need_cancel = state_manager.current_state.need_cancel
        ...
        if need_cancel:
            response_state = {'response': 'I\'m so sorry that I messed up! I\'ll learn to do better next time. Would you please say, cancel, to start over? Or you can say, stop. ', 'shouldEndSession': False}
        else:
            text = state_manager.current_state.text
            intent = state_manager.current_state.final_intent
            last_state_dict = state_manager.user_attributes.last_state_dict
            last_intent = None
            if last_state_dict:
                ...
                last_intent = (
                    state_manager.last_state.get('final_intent', '') if state_manager.last_state is not None else ''
                )
            
            speak_output = select_error_template(text, intent, last_intent)
            
            response = {'response': speak_output, 'shouldEndSession': False}
