...
def execute(user_attributes):
    _, _, total_steps, current_step_num = TacoIntentByRule.parse_step_nums(user_attributes.taco_state)
...
class Taco_halt_Treelet(Treelet):
    ...
    def get_response(self, state_manager, priority=ResponsePriority.WEAK_CONTINUE, **kwargs) -> ResponseGeneratorResult:
        ...
        response_state = execute(state_manager.user_attributes)
