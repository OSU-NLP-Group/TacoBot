...
# treelets
from taco.response_generators.taco_rp.qa.treelets.step_qa_treelet import step_qa_Treelet
from taco.response_generators.taco_rp.qa.treelets.IDK_qa_treelet import IDK_qa_Treelet
# from taco.response_generators.taco_rp.qa.treelets.EVI_qa_treelet import EVI_qa_Treelet
...
NAME2TREELET = {treelet.__name__: treelet for treelet in [step_qa_Treelet, IDK_qa_Treelet]}

print('NAME2TREELET = ', NAME2TREELET)

# response type
ADDITIONAL_RESPONSE_TYPES = ['NEEDED_QA']
ResponseType = add_response_types(ResponseType, ADDITIONAL_RESPONSE_TYPES)
        
logger = logging.getLogger('tacologger')


class QA_Generator(ResponseGenerator):
    name='QA_INTERNT'

    def __init__(self, state_manager) -> None:
        self.step_qa_treelet = step_qa_Treelet(self)
        self.IDK_qa_treelet = IDK_qa_Treelet(self)
        
        treelets = {
            treelet.name: treelet for treelet in [self.step_qa_treelet, self.IDK_qa_treelet]
        }
    ...
    def handle_default_post_checks(self) -> Optional[ResponseGeneratorResult]:
        ...
        treelet = name2initializedtreelet['step_qa_Treelet']
