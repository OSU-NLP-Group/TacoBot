...
from taco.core.experiment import Experiments
...
class State(object):
    ...
    def update_from_last_state(self, last_state):
        ...
        for k in ['status', 'last_status', 'recipe_rec_cat', 'clarify', 'is_rec', 'search_timeout', 'no_result', 'selected_prompt_rg', 'entity_tracker']:

            v = getattr(last_state, k, None)
            if v != None:
                setattr(self, k, v)
