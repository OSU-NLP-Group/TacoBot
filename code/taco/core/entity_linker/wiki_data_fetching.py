...
from taco.core.entity_linker.lists import MANUAL_SPAN2ENTINFO, MANUAL_TALKABLE_NAMES
from taco.core.flags import inf_timeout, use_timeouts
from taco.core.util import query_es_index, get_es_host, get_elasticsearch
...
ANCHORTEXT_QUERY_TIMEOUT = 10.0  # seconds
ENTITYNAME_QUERY_TIMEOUT = 10.0  # seconds
...
def get_entities_by_wiki_name(wiki_names: List[str]) -> Dict[str, WikiEntity]:
    ...
    logger.primary_info(f'Querying "{ARTICLES_INDEX_NAME}" ES index with these {len(wiki_names)} wiki names: {wiki_names}')
    ...
    entities = make_wikientities(results)
    entname2ent = {ent.name: ent for ent in entities}
    logger.primary_info("Got {} WikiEntities:\n{}".format(len(entname2ent), '\n'.join(
