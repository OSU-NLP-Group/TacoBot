...
from taco.annotators.neural_entity_linker import NeuralEntityLinker
from taco.core.asr.search_phone_to_ent import get_asr_aware_span2entsim
from taco.core.callables import Annotator
from taco.core.entity_linker.entity_linker_classes import LinkedSpan, EntityLinkerResult, WikiEntity
from taco.core.entity_linker.util import add_all_alternative_spans
from taco.core.entity_linker.resolve_conflicts import comparison_fn_nested_spans, comparison_fn_alternative_spans, \
    resolve_pairwise_conflicts
from taco.core.entity_linker.thresholds import SCORE_THRESHOLD_HIGHPREC, UNIGRAM_FREQ_THRESHOLD
from taco.core.entity_linker.entity_groups import EntityGroup, ENTITY_GROUPS_FOR_EXPECTED_TYPE
from taco.core.entity_linker.wiki_data_fetching import get_entities_by_anchortext, get_entities_by_wiki_name
from taco.core.entity_linker.lists import LOW_PREC_SPANS, MANUAL_SPAN2ENTINFO, DONT_LINK_WORDS, get_unigram_freq
from taco.core.offensive_classifier.offensive_classifier import contains_offensive
from taco.core.regex.templates import MyNameIsTemplate
...
from taco.core.util import remove_punc, get_ngrams, filter_and_log, make_text_like_user_text, contains_phrase
from taco.core.latency import measure
from taco.core.flags import USE_ASR_ROBUSTNESS_OVERALL_FLAG
...
data_path = os.getcwd()+ "/taco/response_generators/taco_rp/ingredient_qa/data"

with open(data_path + "/all_ingredients.json", "r") as f:
    ALL_INGREDIENTS = json.load(f)
INTENSIFIERS_DICT = {}
for ing in ALL_INGREDIENTS:
    INTENSIFIERS_DICT[ing] = 1
...
def parall_entity_process(span):
    if span in MANUAL_SPAN2ENTINFO:
        return [MANUAL_SPAN2ENTINFO[span].ent_name, 1.0, span]
    else:
        wiki_results = wikipedia.search(span)
        if len(wiki_results) > 0:
            cur_wiki_result = wiki_results[0]
            Partial_Ratio = fuzz.ratio(span.lower(), cur_wiki_result.lower())
            # print('span = ', span, 'cur_wiki_result = ', cur_wiki_result, 'wiki_results = ', wiki_results, 'Partial_Ratio = ', Partial_Ratio)
            if Partial_Ratio > 99:
               return [cur_wiki_result, 1.0, cur_wiki_result]
...
def neural_link_spans(user_utterance: str, context: str, use_asr_robustness: bool, spans: Set[str],
    ...
    output = None
    logger.primary_info(f"user_utterance_to_span_set.keys() {user_utterance_to_span_set.keys()}")
    logger.primary_info(f"user_utterance_to_span_set.values() {user_utterance_to_span_set.values()}")
    ...
    #         elif True:
    #             result.append([span, 1.0, span])

    data_inpts = []

    for utterance, span_set in user_utterance_to_span_set.items():
        for span in span_set:
            data_inpts.append(span)

    # print('data_inpts = ', data_inpts)
    # for utterance, span_set in user_utterance_to_span_set.items():
    #     for span in span_set:
    #         if span in MANUAL_SPAN2ENTINFO:
    #             result.append([MANUAL_SPAN2ENTINFO[span].ent_name, 1.0, span])
    ...
    #                 # print('span = ', span, 'cur_wiki_result = ', cur_wiki_result, 'wiki_results = ', wiki_results, 'Partial_Ratio = ', Partial_Ratio)
    #                 if Partial_Ratio > 99:
    #                    result.append([cur_wiki_result, 1.0, cur_wiki_result])

    with Pool(16) as pool:
      result = pool.map(parall_entity_process, data_inpts)

    result = [rt for rt in result if rt != None]
    # print('result = ', result)
    ...
    # print('linked_spans =' , linked_spans)
...
@measure
def entity_link_ingredient(entity_result, context, expected_type=None, last_bot_utterance=None) -> EntityLinkerResult:
    """
    The top-level entity-linking function. Tries to link all possible spans in user_utterance to entities, and returns
    an EntityLinkerResult.
    ...
    for entity_name, _, _ in entity_result:
        entity_names.append(entity_name)
    ...
    ner_span2type = {}
    entity_name_to_entity = get_entities_by_wiki_name(entity_names)
    for entity_name, confidence, span in entity_result:
        if entity_name in entity_name_to_entity:
            if span in asr_span2origspan:
                span_to_entityname[span].append((entity_name, confidence*asr_entity_info[asr_span2origspan[span]][span]['similarity']))
            else:
...
class EntityLinkerModule(Annotator):
    ...
    def call_cornlp_module(self, input_data):
        def execute_neural():
            responses = CorenlpModule(self.state_manager).execute(input_data)
            return responses
        ...
        corenlp_result = neural_future.result()
    ...
    #     # with futures.ThreadPoolExecutor(max_workers=1, initializer=initializer, initargs=(should_kill,)) as executor:
    #     #     neural_future = executor.submit(execute_neural)

    #     corenlp_result = CorenlpModule(self.state_manager).execute(input_data)
    ...
    def execute(self):
        ...
        try:
            ...
            pak_question_qus_txt = getattr(self.state_manager.user_attributes, 'PAK_selected_qus', None)
            task_ingredients = getattr(self.state_manager.user_attributes, 'current_task_ingredients', None)
            get_ingredient_et = getattr(self.state_manager.user_attributes, 'get_ingredient_et', False)


            pak_entity_link = None
            if pak_question_qus_txt != None:
                input_data = {'text': pak_question_qus_txt, 'annotators': 'pos,ner,parse,sentiment'}
                corenlp_pak = self.call_cornlp_module(input_data)

                pak_entity_link =  entity_link(pak_question_qus_txt, context, corenlp_pak, g2p_module, include_common_phrases=(expected_type is not None),
                                   expected_type=expected_type, neural_entity_linker=neural_entity_linker, last_bot_utterance=last_bot_utterance)

                self.state_manager.user_attributes.PAK_selected_qus = None

            ingredients_entity = None
            if task_ingredients != None and get_ingredient_et != True:
                entity_result = []

                task_ingredients = task_ingredients.split(' <break time="300ms"/>')
                task_ingredients = [tak for tak in task_ingredients]

                n_task_ingredients = []
                for tak in task_ingredients:
                    best_candi = ''
                    for intg in ALL_INGREDIENTS:
                        if intg in tak:
                            if len(best_candi) < len(intg):
                                best_candi = intg

                    if len(best_candi) > 0:
                        n_task_ingredients.append(best_candi)
                
                for span in n_task_ingredients:                
                    wiki_results = wikipedia.search(span)
                    if len(wiki_results) > 0:
                        cur_wiki_result = wiki_results[0]
                        Partial_Ratio = fuzz.ratio(span.lower(), cur_wiki_result.lower())
                        if Partial_Ratio > 99:
                           entity_result.append([cur_wiki_result, 1.0, cur_wiki_result])

                expected_type = ENTITY_GROUPS_FOR_EXPECTED_TYPE.food_related

                ingredients_entity = entity_link_ingredient(entity_result, context, expected_type=expected_type, last_bot_utterance=last_bot_utterance)
                


                self.state_manager.user_attributes.get_ingredient_et = True
            ...
            if self.should_run_entity_linker(user_utterance, history, nav_intent_output):
                cur_entity_link =  entity_link(user_utterance, context, corenlp, g2p_module, include_common_phrases=(expected_type is not None),
                                   expected_type=expected_type, neural_entity_linker=neural_entity_linker, last_bot_utterance=last_bot_utterance)
                if ingredients_entity != None:
                    cur_entity_link.ingrident_high_prec = ingredients_entity.high_prec

                if pak_entity_link != None:
                    cur_entity_link.pak_high_prec = pak_entity_link.high_prec +  pak_entity_link.threshold_removed

                return cur_entity_link
            else:
...
if __name__ == "__main__":
    ...
    from taco.core.latency import save_latency_plot

    # Setup logging with interactive mode logger settings
    from taco.core.logging_utils import setup_logger, LoggerSettings
    ...
    # # Get corenlp
    # from taco.annotators.corenlp import get_nounphrases, get_propernouns, get_ner, get_simple_sentiment
    ...
    from taco.core.asr.index_phone_to_ent import MockG2p
...
#     print('task_ingredients = ', task_ingredients)
...
#     # def entity_link_ingredient(user_utterance: str, context: str, spans_to_lookup = None, g2p_module = None,
#     #             max_ngram: int = MAX_NGRAM_TO_LINK, include_common_phrases: bool = False,
#     #             expected_type: Optional[EntityGroup] = None,
#     #             neural_entity_linker = None,

#     print('ingredients_entity = ', ingredients_entity)
#     pass


            # if 'step' in context.lower():

            #     context_file = context[context.lower().find('step'):]

            #     n_task_ingredients = []
            #     for intg in ALL_INGREDIENTS:
            #         if intg in context_file:
            #             n_task_ingredients.append(intg)
                
            #     print('n_task_ingredients = ', n_task_ingredients)
