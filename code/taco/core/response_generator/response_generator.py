from taco.core.callables import NamedCallable
from taco.core.state_manager import StateManager
from taco.core.regex import response_lists
from taco.core.response_generator.response_type import *
from taco.core.response_generator.neural_helpers import is_two_part, NEURAL_DECODE_CONFIG, get_random_fallback_neural_response
from taco.core.response_generator.state import NO_UPDATE, BaseState, BaseConditionalState
from taco.core.response_generator.neural_helpers import get_neural_fallback_handoff, neural_response_filtering
from taco.core.response_generator.treelet import Treelet
...
from taco.core.response_generator.helpers import *
from taco.core.response_priority import ResponsePriority
from taco.core.util import load_text_file, taco_RP_NAME
...
from taco.response_generators.acknowledgment.acknowledgment_helpers import ACKNOWLEDGMENT_DICTIONARY
from taco.core.entity_linker.entity_groups import ENTITY_GROUPS_FOR_CLASSIFICATION

# from taco.response_generators.music.utils import WikiEntityInterface
...
class ResponseGenerator(NamedCallable):
    ...
    # def set_cur_treelt_attribute(self, attr_name, value):
    #     setattr(self.state_manager.user_attributes, attr_name, value)

    # def get_cur_treelt_attribute(self, attr_name, default):
    ...
    def get_current_entity(self, initiated_this_turn=False):
        ...
        if initiated_this_turn:
            # if current_state.entity_tracker.cur_entity_initiated_by_user_this_turn(self.state_manager):
            if current_state.entity_tracker.cur_entity_initiated_by_user_this_turn(self.state_manager):
                # if self.state_manager.current_state.entity_tracker.entity_initiated_on_turn:
    ...
    def get_entities(self):
        return {
            ...
            'ingredient_untalked': self.state_manager.current_state.entity_tracker.ingredient_untalked,
            'pad_untalked': self.state_manager.current_state.entity_tracker.pad_untalked,
    ...
    def handle_alexa_command(self, slots):
        ...
        return ResponseGeneratorResult(
            text="This is taco bot. I'm happy to chat with you, but I can't execute "
                    "commands like that one. If you want to stop chatting, just say, stop, and you can "
                    f"try your command again. But I'd love to keep talking to you! As I was saying, {last_sentence}",
    ...
    def handle_chatty_phrase(self, slots):
        ...
        response_text_p1 = response_lists.ONE_TURN_RESPONSES.get(slots['chatty_phrase'], "Ok, I'd love to talk to you! What would you like to talk about?")
        # data_cates = random.choices(CATEGORIES_SUP_TOPICS, k=4).
        shuffle(CATEGORIES_SUP_TOPICS)
        # random.choices(list(Set), k = 3)
        
        response_text_p2 = response_text_p1 + ' We can talk about ' + ', '.join(CATEGORIES_SUP_TOPICS[:3]) + ' or ' + CATEGORIES_SUP_TOPICS[4] 
        # response_text += "we can talk about game, animals, history or books"
        # response_text += "we can talk about art, cars, basketball, school, anime, pets, baseball, tv show, cooking, dance, celebrities, travel, hockey"
        
        talked_fin = self.state_manager.current_state.entity_tracker.talked_finished
        talked_rej = self.state_manager.current_state.entity_tracker.talked_rejected
        cur_et = self.state_manager.current_state.entity_tracker.cur_entity
        
        ingredient_untalked = []
        talked_list = [et.name for et in [cur_et] + talked_fin + talked_rej if et]
        for i_et in self.state_manager.current_state.entity_tracker.ingredient_untalked:
            if i_et and i_et.name not in talked_list:
                ingredient_untalked.append(i_et.name)

        shuffle(ingredient_untalked)
        answer_type_rep = AnswerType.NONE

        if len(ingredient_untalked) >= 3:
            response_text_p2 = response_text_p1 + " We can start with the recipe's ingredients like " + ', '.join(ingredient_untalked[:2]) + 'or ' + ingredient_untalked[2] + ', mentioned in the recipe. '
            response_text_p2 += ' Or we can switch topics and talk about ' + ', '.join(CATEGORIES_SUP_TOPICS[:3]) + ' or ' + CATEGORIES_SUP_TOPICS[4] 
            answer_type_rep = AnswerType.QUESTION_CT_INGRED

        return ResponseGeneratorResult(
            text=response_text_p2,
            priority=ResponsePriority.STRONG_CONTINUE,
            needs_prompt=False,
            state=self.state if self.state is not None else self.State(),
            cur_entity=self.get_current_entity(),
            conditional_state=self._construct_no_update_conditional_state(),
            answer_type=answer_type_rep
            )
    ...
    def handle_abrupt_user_initiative(self):
        initiative_handlers = { # NOTE: there's a hand-wavey order to these but no rigorous rules.
            ...
            (lambda: user_requested_repetition(self.state_manager)): self.handle_repetition_request, 
    ...
    def get_response(self, state) -> ResponseGeneratorResult:
        ...
        logger.taco_merge(f"{self.name} identified response_types: {response_types}")
        self.state = state
        ...
        # is_continuing_conversation = (self.get_last_rg_in_control() == self.name and 'NEURAL_CHAT' == self.name)

        is_continuing_conversation = (self.get_last_rg_in_control() == self.name and self.name in taco_RP_NAME)

        if self.state_manager.current_state.parsed_intent == "USERWANTTOCHAT" and ('NEURAL_CHAT' == self.name or 'WIKI' == self.name):
        ...
        if is_continuing_conversation:
            ...
            current_rg_response_handlers = {
                ...
                # (lambda: ResponseType.REQUEST_STOP in response_types): self.handle_user_want_stop,
        ...
        if not is_continuing_conversation: # allow the first branch to divert here
            ...
            activation_check_fns = {
                ...
                (lambda: True): self.handle_direct_navigational_intent, # category
                (lambda: True): self.handle_current_entity, # wiki, food
