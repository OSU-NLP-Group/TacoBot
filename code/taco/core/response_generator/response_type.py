from taco.core.response_generator.regex_templates import *
...
def identify_base_response_types(rg, utterance) -> Set[ResponseType]:
    ...
    if is_trying_to_step(rg, utterance):
        retval.add(ResponseType.REQUEST_STOP)
...
def is_trying_to_step(rg, utterance):
    """
    Classifies whether the user sounds disinterested
    """
    return TryingToStopTemplate().execute(utterance) is not None
