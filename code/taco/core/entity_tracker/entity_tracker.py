...
from taco.core.entity_linker.entity_linker_classes import WikiEntity, EntityLinkerResult
from taco.core.latency import measure
from taco.core.response_generator_datatypes import ResponseGeneratorResult, PromptResult, UpdateEntity, AnswerType
from taco.core.entity_linker.thresholds import SCORE_THRESHOLD_NAV_ABOUT, SCORE_THRESHOLD_NAV_NOT_ABOUT, SCORE_THRESHOLD_EXPECTEDTYPE
...
class EntityTrackerState(object):
    ...
    def __init__(self):
        ...
        # **************** ingredient entity ****************
        # self.mentioned_as_ingredient = []
        self.ingredient_untalked = []
        self.pad_untalked = []
    ...
    def cur_entity_initiated_by_user_this_turn(self, state_manager) -> bool:
        """
        ...
        self.update_from_user(state_manager)

        return self.entity_initiated_on_turn

    def check_current_entities(self):
            # def get_entities(self):
        return {
            'talked_finished': self.talked_finished,
            'talked_rejected': self.talked_rejected,
            'ingredient_untalked': self.ingredient_untalked,
            "pad_untalked": self.pad_untalked,
            'user_mentioned_untalked': self.user_mentioned_untalked,
            'user_topic_unfound': self.user_topic_unfound
        }
    ...
    def finish_entity(self, entity: Optional[WikiEntity], transition_is_possible=True):
        ...
        if entity is not None and entity not in self.talked_finished:
            ...
            # print(f'Putting entity {entity} on the talked_finished list')
    ...
    def update_from_user(self, state_manager):
        ...
        if self.entity_initiated_on_turn is None and last_answer_type in [AnswerType.QUESTION_SELFHANDLING, AnswerType.QUESTION_HANDOFF] and ('PAK' in taco_state or 'chat' in taco_state):
            ...
            if self.entity_initiated_on_turn != self.cur_entity and self.cur_entity is not None:
                if self.cur_entity not in negnav_entities:
                    print('self.cur_entity not in negnav_entities')
        ...
        if nav_intent_output.neg_intent or nav_intent_output.pos_intent or last_answer_type in [AnswerType.QUESTION_SELFHANDLING, AnswerType.QUESTION_HANDOFF] and ('PAK' in taco_state or 'chat' in taco_state):
        ...
        for ls in entity_linker_result.ingrident_high_prec:
            if not self.talked(ls.top_ent):
                logger.info(f'Adding {ls.top_ent} to user_mentioned_untalked')
                self.ingredient_untalked.append(ls.top_ent)

        for ls in entity_linker_result.pak_high_prec:
            if not self.talked(ls.top_ent):
                logger.info(f'Adding {ls.top_ent} to user_mentioned_untalked')
                self.pad_untalked.append(ls.top_ent)
    ...
    def update_from_rg(self, result: Union[ResponseGeneratorResult, PromptResult, UpdateEntity], rg: str, current_state):
        ...
        if isinstance(result, UpdateEntity):
            ...
            # print('result.cur_entity = ', result.cur_entity)
        ...
        # print(new_entity == self.cur_entity, "new_entity = ", new_entity, 'cur_entity = ', self.cur_entity)
...
def test_entity_tracker(user_utterance, bot_utterance='', expected_type=None):
    ...
    from taco.annotators.navigational_intent.navigational_intent import get_nav_intent
    from taco.core.entity_linker.entity_linker import entity_link
...
def main():
    ...
    from taco.core.entity_linker.entity_groups import ENTITY_GROUPS_FOR_EXPECTED_TYPE, EntityGroup
