...
def parse_intent(text, intent, features, allowed_intents, current_taco_state, user_attributes, current_state):
    """
    ...
    Args:
        ...
        features: The features argument passed to select_response_model.
    ...
    print('features = ', features.neuralintent)
    guess_intent = None
    if "neuralintent" in features.__dict__ and features.neuralintent:
        custom_intent, guess_intent = process_custom_intent(features, allowed_intents, current_taco_state) 
    ...
    print("3 regex intent ==========> ", regex_intent)
    # input()
    ...
    if 'TaskChoice' in current_taco_state:
        new_intent = get_task_choice(features, tokens, text, user_attributes)
        if new_intent is not None:
            intent = new_intent
        else:
            new_intent = choice_intent_keywords(text, tokens, features)
            if new_intent is not None:
                intent = new_intent
    ...
    #TODO
        ...
        if intent == 'AMAZON.NextIntent':
            ...
            # TEMP
            if features['punctuation'] and '?' in features['punctuation']:
...
def _transform_text_prep(text):
    ...
    if m:
        ...
        return text, None
def flush_slot_values(features, user_attributes):
    """
    ...
    print('features.recipesearch = ', features.recipesearch)

    if ('recipesearch' in features.__dict__ and features.recipesearch and 'recipename' in features.recipesearch and 
        'raw_extraction' in features.recipesearch['recipename']):
        taskname_recipe = features.recipesearch['recipename']['raw_extraction']


    print('features.tasksearch = ', features.tasksearch)
    
    if ('tasksearch' in features.__dict__ and features.tasksearch and 'taskname' in features.tasksearch and 
        'raw_extraction' in features.tasksearch['taskname']):
        taskname_wikihow = features.tasksearch['taskname']['raw_extraction']

    print('features.tasktype = ', features.tasktype)
...
def process_custom_intent(features, allowed_intents, current_taco_state):
    """
    Process the output of our custom intent module.
    Log the sorted scores and final result to the state table.

    Args:
        features: (dict) the features argument passed to select_response_model.
    ...
    print('features.neuralintent = ', features.neuralintent)
    intent_scores = features.neuralintent['intent_scores']
...
def get_task_choice(features, tokens, text, user_attributes):
    new_intent = None
    task_selection = -1
    
    if 'tasksearch' in features and features['tasksearch'] and 'task_selection' in features['tasksearch']:
        task_selection = features['tasksearch']['task_selection'][0]
    # task selection and recipe selection are the same thing
    # elif task_selection < 0 and 'recipesearch' in features and features['recipesearch'] and 'recipe_selection' in features['recipesearch']:
    #     task_selection = features['recipesearch']['recipe_selection'][0]
    ...
    if task_selection > -1:
        ...
        noun_in_input = features.get('nounphrases', {"user_input_noun_phrases": None}).get("user_input_noun_phrases", None)
...
def choice_intent_keywords(text, tokens, features):
    text = text.lower()
    new_intent = None
    # info_keyword = [
    #     'compare',
...
def expand_wikihow_query(features):
    '''
    Construct expanded queries.
    '''
    task_names = features['taskname']['taskname']
