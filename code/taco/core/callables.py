...
from taco.core import flags
from typing import Dict, List, Optional, Set
from datetime import datetime

from taco.core.test_args import TestArgs
from taco.core.response_priority import ResponsePriority, PromptType
from taco.core.response_generator_datatypes import KILLED_RESULT
...
taco_HOME = os.environ.get('taco_HOME', Path(__file__).parent.parent.parent)
def get_url(name):
...
class RemoteCallable(NamedCallable):
    ...
    def client_fn(self, data):
        try:
            ...
            # logger.taco_merge(f"RemoteCallable {self.name} running with timeout {self.timeout}")
    ...
    def __call__(self, input_data):
        ...
        # logger.taco_merge(f"RemoteCallable {self.name} is sending data {data} with timeout = {self.timeout} seconds to url {self.url}")
        EPS = 0.1
        try:
            # try:
            ...
            if isinstance(response, requests.Response):
                ...
                # logger.taco_merge("RemoteCallable {} finished. result: {}, latency: {}ms".format(self.name, response, (end - start).total_seconds() * 1000))
...
def run_multithreaded(module_instance: List[NamedCallable],
    ...
    UNKILLABLES = ['FALLBACK', 'PERSONAL_ISSUES', 'RED_QUESTION', 'ONE_TURN_HACK', 'NEWS', 'Remote_QA_INTERNT']  # latter is high initiative
...
class Annotator(RemoteCallable):
    def __init__(self, state_manager: StateManager, timeout: float, url: str = None, input_annotations: List[str] = []):
        ...
        timeout = 100
        # print("self.name, timeout = ", self.name, timeout)
        # logger.
        # logger.taco_merge(f"self.name, timeout = {self.name}, {timeout}")
    ...
    def check_user_said_chatty(self):
        if user_said_chatty_phrase(self.state_manager):
            return True
        return False
...
def run_multithreaded_DAG(module_instances: List[Annotator],
    ...
    # logger.taco_merge(f'call remote module timeout {timeout}')
    while unexecuted_modules or future_to_module:
        ...
        for future in done:
            ...
            try:
                ...
                logger.time_track(f"Succesfully executed {module_name}, use time {time_elapsed}")
                succeeded_modules.add(module_name)
            except Exception:
